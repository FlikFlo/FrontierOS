...
import type { BrewLog } from '@/types/database'
...
export default async function BrewDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let brew: BrewLog | null = null

  if (supabase) {
    const { data } = await supabase.from('brew_logs').select('*').eq('id', id).single()
    if (data) brew = data as BrewLog
  }

  if (!brew) {
    brew = mockBrews.find(b => b.id === id) ?? null
  }
  ...
  const isMock = brew.id.startsWith('mock-')
